...
function InvitePlayerToSquad(invitedPly)
	...
	if CurTime() - EFGM.INVITES.lastInviteSentTime < timersInviteCVar:GetInt() then CreateNotification("You can send invites again in " .. timersInviteCVar:GetInt() - math.Round(CurTime() - EFGM.INVITES.lastInviteSentTime, 1) .. " seconds!", MATS.inviteErrorIcon, "ui/error.wav") return end
	...
	if CurTime() - EFGM.INVITES.lastSquadInviteSentTime < timersInviteCVar:GetInt() then
...
net.Receive("PlayerInviteReceive", function(len)
	...
	timer.Simple(timersInviteCVar:GetInt(), function()
		EFGM.INVITES.invitedBy = nil
		EFGM.INVITES.invitedType = nil
	end)
end)
