...
if SERVER then
	...
	function RAID:EndRaid()
		...
		for k, v in player.Iterator() do
			if v:IsInRaid() and v:Alive() then
				v:Kill()
			end
		end
	end
	...
	hook.Add("InitPostEntity", "IntroInit", function()
		...
		-- copying entities (TODO: this is so fucking stupid please find a different way to do this man)
	...
	function IntroGetFreeSpace(spawnGroup)
		if #introSpaces == 0 then return nil end
		...
		for k, spaceInfo in ipairs(shuffledIntroSpaces) do
			...
			if !spaceInfo.occupied then return spaceInfo.animName, k end
		end
