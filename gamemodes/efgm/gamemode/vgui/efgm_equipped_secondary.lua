...
function PANEL:CreateVar(argName, argData, argI)
	...
	wep = EFGM.MENU.Player:GetWeapon(name)
	clip = 0
	clipMax = 0
	mag = ""
	...
	magFont = "PuristaBold18"
	magSizeY = EFGM.MenuScale(19)
	...
	if EFGM.MENU.Player:IsInHideout() then
		...
		if data.tag == nil then
			...
			tagButton.OnClickEvent = function()
				EFGM.MENU.ConfirmTag(name, 0, "equipped", self.SlotID, self.Slot)
			end
		end
	end
...
vgui.Register("EEquippedSecondary", PANEL, "DLabel")
